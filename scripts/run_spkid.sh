...
for cmd in $*; do
   ...
   if [[ $cmd == train ]]; then
       ...
       for dir in $db/BLOCK*/SES* ; do
           ...
           #gmm_train -i 1 -n 5 -v 1 -T 0.001 -N5 -m 15 -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/$name.gmm $lists/class/$name.train || exit 1
           gmm_train -i 0 -n 40 -v 1 -T 0.001 -N16 -m 40 -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/$name.gmm $lists/class/$name.train || exit 1
           echo
       done
   elif [[ $cmd == test ]]; then
	   ...
	   # - The name of the world model will be used by gmm_verify in the 'verify' command below.
        ...
        #gmm_train -i 1 -n 5 -v 1 -T 0.001 -N20 -m 60 -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/$world.gmm $lists/verif/$world.train  || exit 1
        gmm_train -i 0 -v 1 -T 0.001 -N13 -m 40 -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/$world.gmm $lists/verif/$world.train  || exit 1
       ...
       #Cambiar el THRESHOLD cada vez que cambiemos el sistema
        perl -ane 'print "$F[0]\t$F[1]\t";
        if ($F[2] > 1.00639006307311) {print "1\n"}
